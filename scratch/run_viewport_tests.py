...
try:
    ...
    print(f"Connected to page target: {page_target.get('url')} ({ws_url})", flush=True)
    ...
    for w, h, vname in viewports:
        ...
        for pid, pname in pages:
            ...
            if check['hasOverflow']:
                print(f"  [FAIL] {pname} at {w}px: winW={check['winW']}, docW={check['docW']}, badCount={len(check['bad'])}", flush=True)
                for b in check['bad']:
        ...
        card_sym = eval_js(f"""
            (function() {{
                ...
                const items = [
                    ...
                    {{ name: 'intel-card-dark', sel: '.intel-card-dark' }},
                    {{ name: 'recent-scans-card', sel: '.recent-scans-card' }}
                ];
    ...
    for lang in languages:
        ...
        if not lcheck['hasOverflow'] and lcheck['isSymmetric']:
            print(f"  [PASS] Language '{lang}': No overflow (docW={lcheck['docW']}), Intel Card Centered (Left={lcheck['left']}px, Right={lcheck['right']}px)", flush=True)
        else:
