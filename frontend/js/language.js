...
const LanguageModule = {
  ...
  translations: {
    ...
    hi: {
      ...
      validation: { invalidEmail: 'कृपया सही ईमेल पता लिखें।', weakPassword: 'पासवर्ड कम से कम 6 अक्षरों का होना चाहिए।', missingEmail: 'कृपया अपना ईमेल पता लिखें।', wrongPassword: 'ईमेल या पासवर्ड गलत है।', emailInUse: 'यह ईमेल पहले से पंजीकृत है।', network: 'कनेक्ट नहीं हो सका। इंटरनेट कनेक्शन जांचें।', default: 'कुछ गलत हो गया। फिर से प्रयास करें।', resetUnavailable: 'पासवर्ड रीसेट ईमेल नहीं भेजा जा सका।' },
    ...
    ta: {
      ...
      validation: { invalidEmail: 'சரியான மின்னஞ்சலை உள்ளிடவும்.', weakPassword: 'கடவுச்சொல் குறைந்தது 6 எழுத்துகள் இருக்க வேண்டும்.', missingEmail: 'மின்னஞ்சலை உள்ளிடவும்.', wrongPassword: 'மின்னஞ்சல் அல்லது கடவுச்சொல் தவறானது.', emailInUse: 'மின்னஞ்சல் ஏற்கனவே பதிவு செய்யப்பட்டுள்ளது.', network: 'இணைக்க முடியவில்லை.', default: 'ஏதோ தவறு நடந்தது.', resetUnavailable: 'மின்னஞ்சல் அனுப்ப முடியவில்லை.' },
    ...
    te: {
      ...
      validation: { invalidEmail: 'సరైన ఈమెయిల్‌ను నమోదు చేయండి.', weakPassword: 'పాస్‌వర్డ్ కనీసం 6 అక్షరాలు ఉండాలి.', missingEmail: 'ఈమెయిల్ నమోదు చేయండి.', wrongPassword: 'ఈమెయిల్ లేదా పాస్‌వర్డ్ తప్పు.', emailInUse: 'ఈమెయిల్ ఇప్పటికే నమోదు చేయబడింది.', network: 'కనెక్ట్ కాలేకపోయాము.', default: 'ఏదో తప్పు జరిగింది.', resetUnavailable: 'ఈమెయిల్ పంపలేకపోయాము.' },
  ...
  t(path, params = {}) {
    ...
    if (text === undefined) text = getVal(fallback, path);
  ...
  updateDOM() {
    ...
    // Specific navigation & headers
    const navMapping = {
      'button[data-target="view-home"]': 'nav.dashboard',
      'button[data-target="view-scan"]': 'nav.scan',
      'button[data-target="view-weather"]': 'nav.weather',
      'button[data-target="view-market"]': 'nav.market',
      'button[data-target="view-schemes"]': 'nav.schemes',
      'button[data-target="view-history"]': 'nav.history',
      'button[data-target="view-profile"]': 'nav.profile'
    };
    Object.entries(navMapping).forEach(([selector, key]) => {
      document.querySelectorAll(selector).forEach(el => {
        const span = el.querySelector('span:not(.icon):not(.brand-lockup)') || el;
        if (span && !el.classList.contains('brand-container')) span.textContent = this.t(key);
      });
    });
    ...
    // Weather, Market, Schemes updates
